...
For this analysis, we measure the angular distribution of electrons and positrons using the ECal.
Cluster reconstruction was done using the algorithm described in \cite{HPS_PROP}: build clusters around seed hits (hits above a ``seed'' energy threshold and with greater energy than any neighboring hits), and add all neighboring hits above an ``add'' energy threshold.
Hit energy is calibrated by matching track momentum to cluster energy, as described in Appendix \ref{sec:ecal_calib}.
