...
%In addition to this important test of our background simulation, the HPS parasitic tests, staged over the Hall-B Hide run (g14), accomplished the following: 
%\begin{enumerate}
...
The HPS Test Run electromagnetic calorimeter (ECal) consists of 442 lead-tungstate (PbWO$_4$) crystals from the CLAS Inner Calorimeter (IC) with avalanche photodiode (APD) readout~\cite{HPS_PROP_UPD}. 
Crystals in the ECal are arranged in two modules.  There are 5 layers in each module: four layers have 46 crystals and one has 37.  The ECal is mounted downstream of the analyzing dipole magnet at the distance of $\sim 148$~cm from the upstream edge of the magnet. The two ECal modules are positioned just above and below the beam plane such that the edge of the crystal closest to the beam is 3.7~cm away. As was described in Section \ref{setup}, we did not use the vacuum chamber between the two ECal modules for the photon test run. Instead, a 2'' beam pipe was used to transport the photon beam from the pair spectrometer vacuum chamber to HDIce target.  
...
For the HPS Test Run, the ECal made use of the existing low and high voltage systems from the CLAS IC, as well as signal cables and splitters. Connectors on the existing signal cables have been rearranged to accommodate the new layout of the channels. 
 
In Figure \ref{fig:ecal_mounted}, the ECal is shown in its installed position for the parasitic run with photon beams, centered relative to the beam pipe passing between the two modules. Cooling lines, as well as signal, HV and LV cables are visible. 
...
spectrometer (PS) pair converter as a target.  This target,  located $\sim67$~cm from the first layer 
of the tracker, produced a sufficient flux of electron-positron pairs to test the principles of running the HPS experiment.
