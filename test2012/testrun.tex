...
In addition to this important test of our background simulation, the test run accomplished the following goals which are explained in Section \ref{sec:testrun_performance}:
\begin{enumerate}
	\item More than 97\% of SVT channels functioned properly
	\item SVT readout signal to noise of 25.5
	\item SVT hit time resolution of 2.6 ns; this proves time reconstruction will work for HPS
	...
	\item Trigger functions as designed; deficiencies found in test run trigger performance are addressed for the 2014-2015 run
\end{enumerate}
...
The ECal was mounted downstream of the analyzing dipole magnet at the distance of $\sim 148$ cm from the upstream edge of the magnet. The two ECal modules were positioned just above and below the beam pipe such that the edge of the crystal closest to the beam is $3.7$ cm from it. 
...
For the HPS Test Run, the ECal made use of the existing low and high voltage systems from the CLAS IC, as well as signal cables and splitters. Connectors on the existing signal cables were rearranged to accommodate the new layout of the channels. 
