...
reconstructed tracks in the SVT. The main observable of the internal alignment of the silicon sensors is the so-called track residual
which is defined as the difference of the measured hit position and the predicted track 
position at that sensor. Figure~\ref{fig:res_top_nonbend} and~\ref{fig:res_top_bend} 
shows representable3D space point track residuals in the non-bend and bend plane, respectively, 
...
\end{figure*}
These are compared to the residuals from an ideally aligned tracker which are expected 
to be centered on zero, Fig.~\ref{fig:res_top_summary} shows the mean of the residuals for 
...
\end{figure*}
Note the larger width for the later layers which highlights the large 
multiple scattering contribution in the track reconstruction as expected for electrons and 
positrons in this energy range. The intrinsic single hit resolution of $\approx 6~\mu$m is negligible 
for layers beyond the second. 
...
To understand the tracker alignment w.r.t. to the other components in the beam line we 
use the reconstructed tracks and extrapolate them outside the tracking volume. 
By extrapolating the fitted tracks to the converter position we can get a alignment w.r.t. to the beam line. 
Figure ~\ref{fig:extrapol_converter} shows a comparison of the track position at the 
converter which enables us to get a better understanding of the tracker alignment with 
...
\end{figure*}
The luminous region at the converter can be inferred from the photon beam profile HARP 
