...
\emph{*******
The strongest constraints on mediators $A'$ with mass $\gtrsim 200 \MeV/c^2$ come from gamma-ray flux bounds, with large theoretical uncertainties dominated the profile of the Dark Matter halo (which can affect limits from the inner Milky Way and other compact objects by one order of magnitude) and the impact of inner galactic magnetic fields on electrons' energy loss, which reduces the efficiency of gamma-ray production  (leading to up to  factor of 50 weaker limits from the inner Galaxy), see e.g. \cite{Papucci:2009gd}.  Although inner Galaxy constraints  exclude all Sommerfeld-enhanced models for the $e^+e^-$ signal when  a standard Einasto profile and ICS model are assumed, varying \emph{either one} of these within the theoretically accepted range renders these models consistent with gamma-ray bounds.   The constraints from resolved dwarf spheroidals, galaxies, clusters, and CMB fits are all similarly consistent with the Pamela/FERMI $e^+e^-$ fluxes, within uncertainties.  Dark matter annihilation in distant and unresolved galaxies --- expected to exceed the observed diffuse gamma-ray flux by a factor of $\sim1-100$ \cite{Zavala:2011tt}.  The large range of expected fluxes here arises because the total flux is dominated by small subhalos in small galaxies, whose density can only be estimated by fitting density distributions to $N$-body simulation results, then extrapolating 15 orders of magnitude below the simulation's resolution!  So, although this is numerically the most stringent constraint by far, it is far from robust.  }

\emph{In models with light $A'$ ($\lesssim 200 \MeV/c^2$), the dark matter annihilation cross-section increases at low velocity dispersion, thereby increasing the dark matter annihilation cross-sections in dwarf spheroidals, at the time of recombination, and in small, self-bound ``subhalos'' of dark matter relative to annihilation in the local halo or Galactic center.  N-body simulation results suggest that at the Sun's galactocentric radius, self-bound subhalos contribute $O(1)$ contribution to dark matter density-squared (and hence to annihilation rate, if the cross-section were velocity-independent) \cite{Pieri:2009je,Kistler:2009xf,Kamionkowski:2010mi}.  Because these subhalos have low velocity dispersion, their impact is substantial in light-$A'$ models \cite{Slatyer:2011kg}.  It is unclear what fraction of these subhalos are disrupted by interactions with baryons, so we may consider either $O(0)$ or $O(1)$ contributions from substructure to local signals.  If local substructure is insignificant, then models that fit PAMELA/Fermi $e^+e^-$ fluxes with $m_{A'}\lesssim 200 \MeV/c^2$ are robustly excluded by bounds on annihilation in dwarf spheroidals and at recombination.  If local substructure is significant, however, then then models with $m_{A'}\lesssim 200 \MeV/c^2$ are \emph{far less} constrained than those with heavier $A'$: the limits from dwarf spheroidals and recombination are not restrictive, limits from the inner Galaxy are essentially eliminated (because dark matter in the inner Galaxy has higher velocity dispersion, and hence lower annihilation cross-sections, than the local subhalos), and the bound from unresolved galaxies may be weakened by a factor of $2-4$ \cite{Slatyer:2011kg}.  
*********}
